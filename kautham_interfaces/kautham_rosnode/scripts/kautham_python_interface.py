...
from kautham_interfaces.srv import SetPlannerByName
...
def kSetPlannerByName(node, plannername):
    kthsetplannerbyname_client = node.create_client(SetPlannerByName, '/kautham_node/SetPlannerByName')

    req = SetPlannerByName.Request()
    req.name = plannername
    while not kthsetplannerbyname_client.wait_for_service(timeout_sec=1.0):
        node.get_logger().info('SetPlannerByName service not available, waiting again...')
    future = kthsetplannerbyname_client.call_async(req)
    rclpy.spin_until_future_complete(node, future)
    r = future.result()
